...
class PreprocessData(SQL_Generator):    
    ...
    # 
    #def preprocess_time_value(self,time_value: str|dt.datetime) -> str:
    ...
    def preprocess_time_values(self,time_values: pd.Series, percent_format: str) -> pd.Series:
        time_values = time_values.map(
                    lambda x: x.strftime(percent_format) if not pd.isna(x) else 
                    (x if isinstance(x,str) else None)
                    )
        #
        return time_values
    
    def str2datetime_single_value(self, str_value: str) -> Optional[dt.datetime]:
        time_value = None
        if len(str_value) > len(self.date_format):
            time_value = dt.datetime.strptime(str_value,self.timestamp_format) 
        elif len(str_value) == len(self.date_format):
            time_value = dt.datetime.strptime(str_value,self.date_format) 
        #
                
        return time_value
            
    def str2datetime(self,date_values: pd.Series) -> pd.Series:
        robust_data_type = Datatype.robust_get_data_type_from_values(pd.DataFrame(date_values))
        if robust_data_type == "str":
            date_values = date_values.map(lambda x: self.str2datetime_single_value(x) if isinstance(x,str) else None)
        #
    ...
    def preprocess_date_values(self,date_values: pd.Series) -> pd.Series:
        robust_data_type = Datatype.robust_get_data_type_from_values(pd.DataFrame(date_values))
        if robust_data_type == "str":
            date_values = date_values.map(lambda x: x[0,self.date_format])
        
        date_values = self.str2datetime
