...
class SQL_Insert(SQL_Generator):    
    ...
    def prepare_date_series(self,date_series: pd.Series) -> pd.Series:
        date_percent_format = replace_date_format(self.date_format)
        date_series = date_series.map(
    ...
    def prepare_single_column(self,data_series: pd.Series) ->  pd.Series:
        col = data_series.name
        if self.db_dtypes[col].upper() == "DATE":
            data_series = self.prepare_date_series(data_series)
            date_percent_format = replace_date_format(self.date_format)
        ...
        #
        data_series = PrepData.none_for_nan(data_series)
        return data_series
    ...
    def prepare_data_to_insert(self,df: pd.DataFrame ,copy: bool =True) -> list:             
        ...
        for col in list(df.columns):
            new_df[col] = self.prepare_single_column(new_df[col])
        #
