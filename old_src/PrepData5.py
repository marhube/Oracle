...
#Utilty function for "postprocess_column"
def postprocess_all_na_values(all_na_values: pd.Series,db_data_type: str) -> pd.Series:
    if db_data_type == 'NUMBER':
        all_na_values = all_na_values.map(lambda x: np.int64(x) if not pd.isna(x) )
    #
    
    return all_na_values

def postprocess_column(column_values: pd.Series,db_data_type: str) -> pd.Series:
    # Special case if all values are missing
    if pd.Series(column_values.map(lambda x: pd.isna(x))).all():
        column_values = postprocess_all_na_values(column_values, db_data_type)
    elif db_data_type.upper().startswith("DATE("):
        column_values = datetime2date(column_values)
    elif db_data_type.upper() in ["INTEGER","NUMBER(0)"]:
        column_values = column_values.map(lambda x: np.int64(round(x)) if not pd.isna(x) else None)
...
class PreprocessData(SQL_Generator):    
    ...
    def preprocess_time_values(self,time_values: pd.Series, percent_format: str) -> pd.Series:
        time_values = time_values.map(
                    lambda x: x.strftime(percent_format) if not pd.isna(x) else None
                    )
        #
        return time_values
...
class PostprocessData(TableOps):
