...
# Utility function to obtain class of a pandas series
def get_class_of_series(series: pd.Series) -> str:
    return series.dtype.type.__name__
    

def get_column_classes(df: pd.DataFrame) -> dict:
...
def is_datetime_series(series: pd.Series) -> bool:
    return (series.dtype.type.__name__ == 'datetime64')
#    


def datetime2date(df: pd.DataFrame) -> pd.DataFrame:
    print(f'Er nå inne i keep_date_only der type(df.dtypes) er {type(df.dtypes)}')
    print(f"df.dtypes er {df.dtypes} of df.dtypes['col0']  er {df.dtypes['col0']}")
    #
...
class SQL_Insert:
    ...
    def sql_hardcoded_insert(self,df: pd.DataFrame,semicolon: bool=True) -> str:
