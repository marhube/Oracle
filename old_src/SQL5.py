...
class Parser:
    ...
    class StatementHandler:
        def __init__(self,current_block: list = [],execution_order: list = []):
        ...
        def update_statement(self,statement: str) -> 'Parser.StatementHandler':
            ...
            print('Er nå inne i update_statement der statement er')
            print(statement)           
            self.statement = statement.strip()
            print('Før avlevering fra update_statement så er self.statement')
        ...
        def handle_individual_statement(self) -> 'Parser.StatementHandler':
            ...
            print('Er nå inne i  handle_individual_statement der len(self.execution_order) først er')
            print(len(self.execution_order))
            print(f'og self.execution_order er {self.execution_order}')
            print(f' og self.statement er {self.statement}')
           #
            if self.statement:                
                self.execution_order.append(self.statement)
            #
            print('Før avlevering fra handle_individual_statement så er len(self.execution_order)')
            print(len(self.execution_order))
    ...
    def split_on_control_structures(self) -> list:
        ...
        print('Er nå inne i split_on_control_structures der self.sql_query er')
        print(self.sql_query)
        handler = self.StatementHandler(execution_order=[])
        print(f'len(handler.execution_order) er aller først {len(handler.execution_order)}')
        inside_control_structure = False
        
        
        split_statements = [statement.strip()  for statement in self.sql_query.split(';') 
                            if len(statement.strip()) > 0]
        
        print(f'len(split_statements) er {len(split_statements)}')
        print('split_statements[0] er')
        print(split_statements[0])
        # Split the code into statements using semicolons      
        #for statement in self.sql_query.split(';'):
        iter = 0
        for statement in split_statements: 
            print(f'iter er nå {iter} og len(handler.execution_order) er {len(handler.execution_order)}')
            print('statement er først')
            print(statement)              
            print('handler.statement er først')
            print(handler.statement)
            handler.statement =  remove_comments_and_empty_lines(statement)                             
            #
                
            print('handler.statement er nå')
            print(handler.statement)
            ...
            print(f'''len(handler.execution_order) er {len(handler.execution_order)}, 
                  og iter er {iter}''')
            print('handler.execution_order er')
            for enum,statement in enumerate(handler.execution_order):
                print(f'statement {enum}')
                print(statement)
            
            iter = iter + 1 
        #
        print(f'Før alvevering så er len(handler.execution_order) {len(handler.execution_order)}')
