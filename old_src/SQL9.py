...
def find_tablespace(table_name: str) -> str:
    ...
    if is_private_temp_table(table_name):
        tablespace = "USER_PRIVATE_TEMP_TABLES"    
    #
    
    return tablespace

def sql_list_user_tables() -> str:
    str_sql_list_user_tables = '''
    SELECT TABLE_NAME 
    FROM USER_TABLES
    '''
    return remove_comments_and_empty_lines(str_sql_list_user_tables)

def sql_list_user_private_temp_tables() -> str:
    str_sql_list_user_private_temp_tables = '''
    SELECT TABLE_NAME 
    FROM USER_PRIVATE_TEMP_TABLES
    '''
    return remove_comments_and_empty_lines(str_sql_list_user_private_temp_tables)
...
# In Oracle data types can only be obtained from persistent tables. 
# A workaround is to create a persistent table with zero rows which
# is only used to obtain data types and immediately deleted afterwards.
def sql_create_replace_name(table_name: str) -> str:
    replace_name = table_name.replace("_","$")
    return replace_name


def sql_create_replace_table(replace_name: str ,table_name: str) -> str:
    replace_name = table_name.replace("_","$")
    sql_create_representation_table = f'''
    CREATE TABLE "{replace_name}" AS
    (
    SELECT * FROM "{table_name}"
    WHERE rownum < 0
    )
    '''    
    return remove_comments_and_empty_lines(sql_create_representation_table)
...
def sql_data_types_user_table(table_name: str) -> str:
    str_data_types_user_table = sql_search_user_table(table_name)
    #
    str_data_types  = f"""
    SELECT COLUMN_NAME,
    CASE 
    ...
    WHERE table_name = ({str_data_types_user_table})
    """    
...
#Memo to self: "sql_find_user_int_columns" only works for persistent user tables
def sql_find_user_int_columns(table_name:str) -> str:
    str_find_int_columns  = f"""
    SELECT COLUMN_NAME FROM  USER_TAB_COLUMNS
    WHERE UPPER(TABLE_NAME)  = '{table_name.upper()}' AND
    DATA_TYPE = 'NUMBER' AND DATA_SCALE = 0
    """
    return remove_comments_and_empty_lines(str_find_int_columns)
...
class SQL_Generator:
    ...
    def __init__(
        ...
        #"date_cols" is for mandating certain columns to have date format inside Oracle
        date_cols: list = []
        ):        
    ...
    def map2db_dtype(self) -> dict:
        ...
        for key,value in self.dtypes.items():
            oracle_data_type = dtype_mapping[value]
            if key in self.date_cols:
                oracle_data_type= "DATE"
                
            oracle_dtypes[key] = oracle_data_type
