...
class Oracle_Connect:
    ...
    def create_or_replace_persistent_table(self,df: pd.DataFrame ,table_name: str,**kwargs) -> None:
        ...
        str_create_table_commit = sql_create.sql_create_table()
        str_insert_into_commit = sql_insert.sql_insert_into()
        #
...
class SQL_Insert:
    ...
    def conditional_col_transform(self,
        ...
        if hardcode_value:          
            ...
            # mypy skjønner ikke at hvis de to første betingelsene er oppfylt så er
    ...
    def none_for_nan(df: pd.DataFrame) -> pd.DataFrame:   
        # Skal nå endre til å bruke "nye måter å besteme klasse på"
        # Iterate through each column to check its dtype
        print('Er nå inne i noneo_for_nan')
        for col in df.columns:
            if get_class_of_series(df[col])  in ['object_','datetime64']:
                # Replace np.nan with None for 'object_' or datetime64 columns
    ...
    def prepare_data_to_insert(self,df: pd.DataFrame ,copy: bool =True) -> list:             
        ...
        for col in list(df.columns):
            if self.sql_generator.db_dtypes[col].upper() == "DATE" and self.sql_generator.datetime2date:
                print(f'Kommer inn i if-en der df[col] er {df[col]} ')
                ...
                print(f'Etter transformasjonen så er new_df {new_df}')
        #
